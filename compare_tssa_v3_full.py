"""
TSSA 3.0 Universal Benchmark Gate: 4-Way Comprehensive Comparison Reporter
Compares newly trained TSSA 3.0 models (in checkpoints/tssa_v3/) against:
1. Baseline Vanilla Models (BARTpho & ViT5)
2. Legacy TSSA v1 Models (Archived in checkpoints/backup_tssa_legacy_v1/ or Official Results)
3. TSSA 2.1 Pilot Models (in checkpoints/)
4. TSSA 3.0 Universal Win Models (in checkpoints/tssa_v3/)
...
def main():
    print("=" * 128)
    print("     📊 BÁO CÁO ĐỐI SOÁT 4 CHIỀU TOÀN DIỆN: TSSA 3.0 vs TSSA 2.1 vs TSSA v1 vs VANILLA BASELINE")
    print("=" * 128)
    ...
    for lang in languages:
        ...
        d_vanilla_bleu = (v3_bleu - v_bleu) if v3_bleu is not None else None
        d_v2_bleu = (v3_bleu - v2_bleu) if (v3_bleu is not None and v2_bleu is not None) else None

        d_vanilla_chrf = (v3_chrf - v_chrf) if v3_chrf is not None else None
        d_v2_chrf = (v3_chrf - v2_chrf) if (v3_chrf is not None and v2_chrf is not None) else None
    ...
    print("\n--- [BẢNG 1: SacreBLEU (ĐỘ ĐO CHÍNH)] ---")
    header_bleu = f"{'MÔ HÌNH':<9} | {'NGÔN NGỮ':<15} | {'VANILLA':<8} | {'TSSA v1':<8} | {'TSSA 2.1':<8} | {'TSSA 3.0':<8} | {'Δ vs VANILLA':<13} | {'Δ vs 2.1':<10} | {'TRẠNG THÁI'}"
    print(header_bleu)
    print("-" * 128)
    ...
    for r in rows:
        ...
        v3_str = f"{r['v3_bleu']:.2f}" if r['v3_bleu'] is not None else "Đang chờ"
        dv_str = f"{r['d_vanilla_bleu']:+.2f}" if r['d_vanilla_bleu'] is not None else "--"
        d2_str = f"{r['d_v2_bleu']:+.2f}" if r['d_v2_bleu'] is not None else "--"

        status = "⏳ Chưa có số liệu"
        if r['v3_bleu'] is not None:
            any_done = True
        ...
        print(f"{r['backbone']:<9} | {r['lang']:<15} | {r['vanilla_bleu']:<8.2f} | {r['legacy_bleu']:<8.2f} | {v2_str:<8} | {v3_str:<8} | {dv_str:<13} | {d2_str:<10} | {status}")
    ...
    # Print chrF++ Table
    print("\n--- [BẢNG 2: chrF++] ---")
    header_chrf = f"{'MÔ HÌNH':<9} | {'NGÔN NGỮ':<15} | {'VANILLA':<8} | {'TSSA v1':<8} | {'TSSA 2.1':<8} | {'TSSA 3.0':<8} | {'Δ vs VANILLA':<13} | {'Δ vs 2.1':<10} | {'TRẠNG THÁI'}"
    print(header_chrf)
    print("-" * 128)

    for r in rows:
        v2_str = f"{r['v2_chrf']:.2f}" if r['v2_chrf'] is not None else "--"
        v3_str = f"{r['v3_chrf']:.2f}" if r['v3_chrf'] is not None else "Đang chờ"
        dv_str = f"{r['d_vanilla_chrf']:+.2f}" if r['d_vanilla_chrf'] is not None else "--"
        d2_str = f"{r['d_v2_chrf']:+.2f}" if r['d_v2_chrf'] is not None else "--"

        status = "⏳ Chưa có số liệu"
        if r['v3_chrf'] is not None:
            if r['d_vanilla_chrf'] > 0:
                status = "✅ Tăng chrF++"
            else:
                status = "❌ Giảm chrF++"

        print(f"{r['backbone']:<9} | {r['lang']:<15} | {r['vanilla_chrf']:<8.2f} | {r['legacy_chrf']:<8.2f} | {v2_str:<8} | {v3_str:<8} | {dv_str:<13} | {d2_str:<10} | {status}")
