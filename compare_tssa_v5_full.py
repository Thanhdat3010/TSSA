"""
UniTSSA 5.0 Benchmark Gate: 6-Way Comprehensive Comparison Reporter (NAACL Gold Standard)
Compares newly trained UniTSSA 5.0 models (in checkpoints/tssa_v5/) against:
...
# Official reference numbers from docs/OFFICIAL_EXPERIMENT_RESULTS.md and benchmark logs (5 epochs, seed 42)
...
def main():
    print("=" * 155)
    print("     📊 BÁO CÁO ĐỐI SOÁT 6 CHIỀU TOÀN DIỆN: UniTSSA 5.0 vs UniTSSA 4.0 vs TSSA 3.0 vs TSSA 2.1 vs TSSA v1 vs VANILLA")
    print("=" * 155)
    ...
    for lang in languages:
        ...
        v5_bleu = v5_metrics["bleu"] if v5_metrics else None
        ...
        v5_chrf = v5_metrics["chrf"] if v5_metrics else None

        d_vanilla_bleu = (v5_bleu - v_bleu) if v5_bleu is not None else ((v4_bleu - v_bleu) if v4_bleu is not None else None)
        d_vanilla_chrf = (v5_chrf - v_chrf) if v5_chrf is not None else ((v4_chrf - v_chrf) if v4_chrf is not None else None)
    ...
    header_bleu = f"{'MÔ HÌNH':<8} | {'NGÔN NGỮ':<15} | {'VANILLA':<8} | {'TSSA v1':<8} | {'TSSA 2.1':<8} | {'TSSA 3.0':<8} | {'UniTSSA 4.0':<11} | {'UniTSSA 5.0':<11} | {'Δ vs VANILLA':<13} | {'TRẠNG THÁI'}"
    print(header_bleu)
    print("-" * 155)
    ...
    for r in rows:
        ...
        v5_str = f"{r['v5_bleu']:.2f}" if r['v5_bleu'] is not None else "Đang chờ"
        dv_str = f"{r['d_vanilla_bleu']:+.2f}" if r['d_vanilla_bleu'] is not None else "--"

        status = "⏳ Chưa có số liệu"
        active_score = r['v5_bleu'] if r['v5_bleu'] is not None else r['v4_bleu']
        if r['v5_bleu'] is not None:
            any_v5_done = True

        if active_score is not None:
            delta = active_score - r['vanilla_bleu']
            prefix = "[v5] " if r['v5_bleu'] is not None else "[v4] "
            if delta >= 0.20:
                status = f"{prefix}✅ THẮNG ĐẬM (>= +0.2)"
            elif delta > 0:
                status = f"{prefix}✅ THẮNG (Tăng vs Vanilla)"
            else:
                status = f"{prefix}❌ THUA vs Vanilla"
                all_win = False
        else:
            all_win = False

        print(f"{r['backbone']:<8} | {r['lang']:<15} | {r['vanilla_bleu']:<8.2f} | {r['legacy_bleu']:<8.2f} | {v2_str:<8} | {v3_str:<8} | {v4_str:<11} | {v5_str:<11} | {dv_str:<13} | {status}")

    print("-" * 155)

    if any_v5_done and all_win:
        print("\n🎉 [KẾT QUẢ TỔNG THỂ] 100% CẢ 6 MÔ HÌNH ĐỀU THẮNG VANILLA BASELINE (ALL GREEN - NAACL READY)!")
    elif any_v5_done:
        print("\n[*] Đang trong quá trình hoàn thiện các mô hình UniTSSA 5.0 còn lại.")

    # Print chrF++ Table
    print("\n--- [BẢNG 2: chrF++] ---")
    header_chrf = f"{'MÔ HÌNH':<8} | {'NGÔN NGỮ':<15} | {'VANILLA':<8} | {'TSSA v1':<8} | {'TSSA 2.1':<8} | {'TSSA 3.0':<8} | {'UniTSSA 4.0':<11} | {'UniTSSA 5.0':<11} | {'Δ vs VANILLA':<13} | {'TRẠNG THÁI'}"
    print(header_chrf)
    print("-" * 155)
    ...
    for r in rows:
        ...
        v5_str = f"{r['v5_chrf']:.2f}" if r['v5_chrf'] is not None else "Đang chờ"
        dv_str = f"{r['d_vanilla_chrf']:+.2f}" if r['d_vanilla_chrf'] is not None else "--"

        status = "⏳ Chưa có số liệu"
        active_score = r['v5_chrf'] if r['v5_chrf'] is not None else r['v4_chrf']
        if active_score is not None:
            delta = active_score - r['vanilla_chrf']
            prefix = "[v5] " if r['v5_chrf'] is not None else "[v4] "
            if delta > 0:
                status = f"{prefix}✅ Tăng chrF++"
            else:
                status = f"{prefix}❌ Giảm chrF++"

        print(f"{r['backbone']:<8} | {r['lang']:<15} | {r['vanilla_chrf']:<8.2f} | {r['legacy_chrf']:<8.2f} | {v2_str:<8} | {v3_str:<8} | {v4_str:<11} | {v5_str:<11} | {dv_str:<13} | {status}")
