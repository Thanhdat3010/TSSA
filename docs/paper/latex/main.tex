...
(4) \textbf{Embedding Alignment:} AWESOME-align~\cite{dou2021word} (fine-tuned directly on in-domain bilingual parallel splits following self-training objectives);
(5) \textbf{Contrastive Representation:} Cross-Lingual InfoNCE (CL-LSA)~\cite{chi2021infoxlm}.

\subsection{Training and Evaluation Protocol}
All models across all paradigms are trained under a strictly controlled computational budget using Seed 42 with deterministic PyTorch CUDA kernels (\texttt{torch.manual\_seed(42)}, \texttt{torch.use\_deterministic\_algorithms(True)}) on a single NVIDIA RTX 4090 GPU (24GB VRAM) for 20 epochs with early stopping (patience 5) based on validation chrF++. We use the AdamW optimizer ($\beta_1=0.9, \beta_2=0.98, \epsilon=10^{-8}$) with a linear warmup of 1,000 steps followed by polynomial decay, and an effective batch size of 32 via gradient accumulation. Crucially, the Online Target Teacher $\mathcal{E}_T$ adds only $\approx 11.4\%$ training compute (measured as wall-clock training time per epoch compared to unregularized Vanilla backbone fine-tuning on the same single NVIDIA RTX 4090 GPU under identical batch size) and is completely discarded prior to inference, resulting in \textbf{exactly zero additional parameters and zero latency overhead during inference}.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Isolating Tay Dominance:} On Tay, TSSA achieves \textbf{25.32 BLEU} on BARTpho (+0.65 over Vanilla) and reaches \textbf{35.97 BLEU (+0.98)} on ViT5, achieving top-tier BLEU (35.97) while maintaining competitive chrF++ (45.42).
\end{itemize}
